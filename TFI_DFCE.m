...
%             ber_result(ebno)=ber_result(ebno)+BER;
            ...
            for Phase=1:phase
                ...
                %チャネル補償
                received_signal_a=frequency_signal(number_pilot+1:number_symbol+number_pilot,:)./channel_resp_dec_a;%channel_resp_dec_re;
...
%     view(net);%ネットワークの仕組みを表示
    ...
    legend('TFI-GRNN')
    title('BER');
    semilogy(ebno_min:ebno_step:ebno_max,ber_result(:,2)','--b*');
    xlabel('Eb/No[dB]');
    ylabel('BER');
    axis([0,30,10^(-6),10^(0)]);
    grid;
